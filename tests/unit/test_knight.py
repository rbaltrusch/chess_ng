...
from chess_ng.piece import Knight, Pawn
...
def test_knight_capture(position, team, expected):
    ...
    piece2 = Pawn(
        direction=1, position=convert_str(position), representation=f"o{team}"
    )
...
def test_knight_move_through_targets():
    ...
    pawns = [
        Pawn(direction=1, position=convert_str(pos), representation="o2")
